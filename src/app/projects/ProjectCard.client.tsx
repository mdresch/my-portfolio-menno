  ...
  return (
    <div className="border rounded-lg p-6 shadow-lg hover:shadow-xl transition-shadow">
      <Image
        ...
        className="mb-4 h-auto w-full max-h-[250px] rounded-lg object-cover"
        style={{ height: "auto" }}
      />
...
import React from "react";
import { useRouter } from "next/navigation";
import ProjectCardServer, { Project as ProjectType } from "./ProjectCard.server";
...
export default function ProjectCardClient({ project, onEdit }: ProjectCardProps) {
  const router = useRouter();
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex gap-2">
        ...
        {onEdit ? (
          <button className="text-gray-600" onClick={onEdit}>Select</button>
        ) : null}
      </div>
    </div>
  );
