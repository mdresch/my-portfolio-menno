...
export default function GitHubActivity() {
  ...
  function formatEventText(event: GitHubEvent): string {
    ...
    case "PullRequestEvent":
      return `${event.payload.action} PR: ${event.payload.pull_request?.title} in ${event.repo.name}`;
    case "CreateEvent":
    ...
    <div className="space-y-6">
      ...
      <div className="border-t pt-4">
        {filteredEvents.length === 0 ? (
          <p className="text-gray-500 text-center py-4">No {filter !== "all" ? filter : ""} activity found</p>
        ) : (
          <div className="space-y-2">
            {filteredEvents.map((event) => (
              <div key={event.id} className="flex items-start gap-3 text-sm hover:bg-gray-50 p-2 rounded-lg transition-colors">
                <svg
