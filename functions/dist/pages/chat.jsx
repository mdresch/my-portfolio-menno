...
export default function ChatPage() {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            setMessages((msgs) => msgs.map((msg, i) => i === msgs.length - 1
    ...
    return (<div className="flex flex-col h-screen bg-gray-100">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto px-4 py-6 space-y-6 bg-gray-50">
          {messages.length === 0 && (<div className="text-center text-gray-400 mt-10">No messages yet. Start the conversation!</div>)}
          {messages.map((msg, i) => (<div key={i}>
              ...
              {msg.ai && (<div className="flex justify-start mb-2">
                  <div className="flex flex-col items-start">
                    ...
                    <div className={`mt-1 rounded-lg px-4 py-2 max-w-xs whitespace-pre-wrap break-words shadow-md ${msg.error
        ...
        {/* Error message */}
        {error && (<div className="bg-red-100 text-red-700 text-center py-2 border-t border-red-200">
            {error}
          </div>)}
        {/* Prompt input area */}
        <form className="bg-white p-4 flex gap-2 border-t shadow-lg justify-center" style={{
            position: 'fixed',
            bottom: 0,
            left: 0,
            right: 0,
          ...
          <div className="w-full max-w-2xl flex gap-2 mx-auto">
            <TextareaAutosize className="border rounded flex-1 p-2 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none" value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => {
            if (e.key === 'Enter' && !e.shiftKey && !loading) {
                e.preventDefault();
                sendMessage();
            }
        }} placeholder="Type your message..." disabled={loading} autoFocus minRows={1} maxRows={12} // About half the screen on most devices
    />
            <button className="bg-blue-600 hover:bg-blue-700 text-white p-2 rounded flex items-center justify-center transition disabled:opacity-50" type="submit" disabled={loading || !input.trim()} aria-label="Send">
